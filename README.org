...
* Levy Flights
Why are we giving the lower limit of bit flips as 5? Can't it be 0 to generate true randomness?
#+begin_src julia
...
Currently it is using repair strategy for fitness value calculation. Can be tried with maximum profit-over-weight-ratio penalty or death penalty to see its effect on solution quality and convergence speed.
#+begin_src julia
...
* Search Algorithm
Need to check impact by increasing the number of iterations
#+begin_src julia
...
Maybe play around with parameters a but more? (also, check the impact of phase angle on the code performance)
#+begin_src julia
